...
(function () {
    // =============================================================
    // # SECTION: Node class name / constants
    // =============================================================
    const NODE_CLASS = "FluxSettingsPipe";

    // =============================================================
    // # SECTION: PresetsAPI (fetch/save/delete + cache)
    // =============================================================
    const PresetsAPI = (() => {
        ...
        function list() {
            return presetCache.slice();
        }

        // prime the cache (non-blocking for the node lifetime)
        refresh();
    ...
    // =============================================================
    // # SECTION: Utils (values <-> widgets, colors, current preset)
    // =============================================================
    const Utils = (() => {
        function getNodeValues(node) {
            ...
            for (const w of widgets) {
                if (!w) continue;
                if (w.__fluxHeaderMarker) continue; // headers
                if (w.name === "__hdr_PRESET_FOOTER") continue; // footer
                if (w.type === "button" && w.name === "manage_presets") continue; // UI button
        ...
        // -- localStorage helpers for section colors
        const LS_SECTION_COLORS = "flux_settings_pipe.section_colors";
        function _colorsLoad() {
            try {
        ...
        function colorsGet(key, fallback) {
            const map = _colorsLoad();
            return map[key] || fallback;
        }
        function colorsSet(key, color) {
            const map = _colorsLoad();
            map[key] = color;
            localStorage.setItem(LS_SECTION_COLORS, JSON.stringify(map));
        }
        ...
        // -- localStorage for current preset per node
    ...
    // =============================================================
    // # SECTION: SectionHeaders (draw colored headers in node)
    // =============================================================
    const SectionHeaders = (() => {
        function insertHeader(node, beforeWidgetName, label, bgColor, markerName) {
                ...
                w = node.addWidget(
                    ...
                    () => {
                        Utils.openColorPicker(w._bg, (val) => {
                            if (!val) return;
                            w._bg = val;
                            w._fg = Utils.textColorFor(val);
                            Utils.colorsSet(marker, val);
                            node.setDirtyCanvas(true, true);
                        });
                    },
                    { serialize: false }
            ...
            w._bg = stored;
            w._fg = Utils.textColorFor(stored);
    ...
    const Footer = (() => {
        function ensureFooter(node) {
            ...
            w.name = marker;
            w._bg = DEFAULT_BG;
            w._fg = Utils.textColorFor(DEFAULT_BG);
    ...
    // =============================================================
    // # SECTION: ManageDialog (UI to manage presets)
    // =============================================================
    const ManageDialog = (() => {
        ...
        function createManageDialog(node) {
            ...
            fillOptions();
            PresetsAPI.refresh().then(fillOptions);
            refreshBtn.onclick = () => {
                PresetsAPI.refresh().then(fillOptions);
            };
            ...
            button("Apply to node", () => {
                ...
                if (p?.payload) {
                    Utils.setNodeValues(node, p.payload);
                    Footer.setFooterPresetName(node, nm);
                }
            });
            ...
            button("Rename…", async () => {
                ...
                const found = PresetsAPI.list().find((p) => p.name === old);
                if (!found) return;
                const res1 = await PresetsAPI.savePreset(neu, found.payload);
                if (!res1?.ok) {
                    alert("Failed to rename (save-as)");
                    return;
                }
                const res2 = await PresetsAPI.deletePreset(old);
                if (!res2?.ok) {
                    alert("Failed to rename (delete old)");
                    return;
                }
                await PresetsAPI.refresh();
                fillOptions();
                Footer.setFooterPresetName(node, neu);
            ...
            button("Delete", async () => {
                ...
                if (res?.ok) {
                    if (Utils.loadCurrentPresetName(node) === nm) Footer.setFooterPresetName(node, null);
                    await PresetsAPI.refresh();
                    fillOptions();
            ...
            button("Import", () => {
                ...
                inp.onchange = async () => {
                    ...
                    try {
                        ...
                        // Compat with grouped preset format { presets: { nodeType: [{ name, values/payload }] } }
                        if (obj?.presets && typeof obj.presets === "object") {
                            ...
                            for (const it of all) {
                                await PresetsAPI.savePreset(it.name, it.payload);
                            }
                            await PresetsAPI.refresh();
                            fillOptions();
                            app.ui?.toast?.("Presets imported");
                        } else {
                            const nm = obj?.name || prompt("Preset name for import:");
                            const payload = obj?.payload || obj; // fallback to raw object
                            if (!nm || !payload) {
                                alert("Invalid file");
                                return;
                            }
            ...
            closeBar.style.justifyContent = "flex-end";
            closeBar.style.marginTop = "10px";
    ...
    // =============================================================
    // # SECTION: Integration (register extension + nodeCreated)
    // =============================================================
    const Integration = (() => {
        function ensureManageButton(node) {
            ...
            btn.draw = function (ctx, node2, widgetWidth, y, height) {
                ...
                ctx.strokeRect(0.5, y + 2.5, widgetWidth - 1, h - 5);
                ctx.restore();
                ctx.save();
                ctx.font = "700 12px sans-serif";
                ctx.textBaseline = "middle";
        ...
        function register() {
            app.registerExtension({
                ...
                async beforeRegisterNodeDef(nodeType, nodeData, appRef) {
                    ...
                    // ---- extend context menu with Presets submenu
                        ...
                        const loadSub = names.length
                            ? names.map((nm) => ({
                                ...
                                callback: () => {
                                    const p = PresetsAPI.list().find((x) => x.name === nm);
                                    if (p?.payload) {
                                        Utils.setNodeValues(this, p.payload);
                                        Footer.setFooterPresetName(this, nm);
                                    }
                                },
                            }))
                        ...
                        const delSub = names.length
                            ? names.map((nm) => ({
                                ...
                                callback: async () => {
                                    ...
                                    if (cur === nm) Footer.setFooterPresetName(this, null);
                                },
                            }))
                            : [{ content: "No presets", disabled: true }];
